...
class CaseStudyRequest(BaseModel):
    ...
    user_prompt: str = Field(
        ...,
        description="The user's full request for a case study, which should mention the target company.",
    )
...
def extract_company_from_prompt(prompt: str) -> Optional[str]:
    ...
    patterns = [r"(?:at|from|for|with)\s+([A-Z][a-zA-Z0-9]*(?:\s+[A-Z][a-zA-Z0-9]*)*)"]
...
async def find_company_domain_via_search(company_name: str) -> Optional[str]:
    ...
    try:
        ...
        if response.results:
            ...
            if not domains:
                logger.warning(
                    f"Could not extract any valid domains from pre-search for '{company_name}'."
                )
                return None
...
async def case_studies_search(request: CaseStudyRequest):
    ...
    if company_name:
        ...
        if not target_domain:
            ...
            logger.warning(
                f"Could not dynamically find domain for '{company_name}', falling back to .com guess."
            )
    ...
    try:
        llm_prompt = GENERATE_SEARCH_QUERY_PROMPT_TEMPLATE.format(
        ...
        raise HTTPException(
            status_code=500, detail="Failed to generate search query from LLM."
        )
    ...
    try:
        ...
        if not response.results:
            logger.warning(
                f"No case studies found for domain '{target_domain}' with query '{scoped_query}'"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail="An unexpected error occurred while searching for case studies.",
        )
