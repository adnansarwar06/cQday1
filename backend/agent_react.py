...
class ReActAgentExecutor:
    ...
    async def _execute_step(
        ...
        async for chunk in llm_stream:
            ...
            if "```json" in current_chunk_buffer and not in_json_block:
                ...
                if not (
                    '"tool_name"' in chunk
                    or '"tool_input"' in chunk
                    or chunk.strip().startswith("{")
                ):
        ...
        if current_chunk_buffer and not in_json_block:
            ...
            if not (
                '"tool_name"' in current_chunk_buffer
                or '"tool_input"' in current_chunk_buffer
                or current_chunk_buffer.strip().startswith("{")
            ):
        ...
        if not action_match:
            ...
            has_previous_actions = any(
                "Action:" in step or "Observation:" in step for step in self.scratchpad
            )
            ...
            looks_incomplete = any(
                phrase in cleaned_response.lower() for phrase in incomplete_phrases
            )
            ...
            looks_complete = any(
                indicator in cleaned_response.lower()
            ...
            if (
                has_previous_actions and is_substantial and not looks_incomplete
            ) or looks_complete:
        ...
        try:
            ...
            if tool_name in self.tools:
                ...
                if tool_name == "web_search":
                    ...
                    yield {
                        "type": "action_progress",
                        "content": f"Finding case studies for: '{query}'...\n",
                    }
                ...
                if tool_name == "web_search" and hasattr(result, "results"):
                    yield {
                        "type": "action_progress",
                        "content": f"✅ Found {len(result.results)} search results\n",
                    }
                    await asyncio.sleep(0.3)
                    yield {
                        "type": "action_progress",
                        "content": f"🔄 Processing and scraping websites...\n",
                    }
                    await asyncio.sleep(0.3)
                    ...
                    for i, search_result in enumerate(result.results[:5], 1):
                        ...
                        title = (
                            search_result.title[:60] + "..."
                            if len(search_result.title) > 60
                            else search_result.title
                        )
                        # Show scraping progress for each site
                        yield {
                            "type": "action_progress",
                            "content": f"🔄 Scraping [{i}/{min(5, len(result.results))}]: {title[:40]}...\n",
                        }
                    ...
                    yield {
                        "type": "action_progress",
                        "content": f"✅ Completed scraping {min(5, len(result.results))} websites\n",
                    }
                    await asyncio.sleep(0.3)
                ...
                # Provide detailed progress for case studies search
                    ...
                    yield {
                        "type": "action_progress",
                        "content": f"✅ Found {len(result.results)} case studies\n",
                    }
                    await asyncio.sleep(0.3)
                    yield {
                        "type": "action_progress",
                        "content": f"🔄 Processing case studies...\n",
                    }
                    await asyncio.sleep(0.3)
                    ...
                    for i, study in enumerate(result.results[:3], 1):
                        ...
                        yield {
                            "type": "action_progress",
                            "content": f"📚 Analyzing [{i}/{min(3, len(result.results))}]: {title}\n",
                        }
                        await asyncio.sleep(0.5)
                        yield {
                            "type": "action_progress",
                            "content": f"• [{i}] {study.title}\n   📍 {study.url}\n",
                        }
                        await asyncio.sleep(0.3)

                    yield {
                        "type": "action_progress",
                        "content": f"✅ Completed analyzing {min(3, len(result.results))} case studies\n",
                    }
                    await asyncio.sleep(0.3)
            ...
            error_message = (
                "I had trouble parsing my action. Let me think about this differently."
            )
    ...
    def _format_observation(self, tool_name: str, result) -> str:
        ...
        if tool_name == "case_studies_search":
            ...
            clean_result = (
                clean_result.replace("{", "").replace("}", "").replace('"', "")
            )
            # Convert \n to actual newlines
            clean_result = (
        ...
        if hasattr(result, "results") and result.results:
            ...
            for i, study in enumerate(result.results[:3], 1):  # Show top 3
                ...
                if study.summary:
                    ...
                    clean_summary = clean_summary.replace("<strong>", "**").replace(
                        "</strong>", "**"
                    )
                    clean_summary = clean_summary.replace("<b>", "**").replace(
                        "</b>", "**"
                    )
                    clean_summary = clean_summary.replace("<em>", "*").replace(
                        "</em>", "*"
                    )
                    clean_summary = clean_summary.replace("<i>", "*").replace(
                        "</i>", "*"
                    )
    ...
    def _format_file_content(self, result) -> str:
        ...
        if hasattr(result, "content"):
            ...
            filepath = (
                str(result.filepath).replace("{", "").replace("}", "")
                if hasattr(result, "filepath")
                else "file"
            )
            size_info = (
                f" ({result.size_bytes} bytes)" if hasattr(result, "size_bytes") else ""
            )
    ...
    def _format_file_list(self, result) -> str:
        ...
        if hasattr(result, "files") and result.files:
            files_list = "\n".join(
                f"• {filename}" for filename in result.files[:20]
            )  # Show more files
            ...
            total_text = (
                f"\n\n(Showing {min(20, len(result.files))} of {result.total_count} files)"
    ...
    async def run(self, user_prompt: str) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        for i in range(self.max_steps):
            ...
            yield {
                "type": "final_answer",
                "content": "I've reached my maximum number of reasoning steps. Let me provide you with what I've found so far.",
            }


async def run_react_agent_streaming(
    request: ReActAgentRequest, tools: Optional[Dict[str, Tool]] = None
) -> AsyncGenerator[Dict[str, Any], None]:
    """
