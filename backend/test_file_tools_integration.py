...
async def test_file_tools_as_agent_tools():
    ...
    file_tools = {name: tool for name, tool in TOOL_REGISTRY.items() if name.startswith(('list_files', 'read_file', 'create_file', 'edit_file'))}
    ...
    request = AgentRequest(user_prompt="Read the content of knowledge_base/sample_document.txt")
    response = await route_request(request, tools=file_tools)
    print(f"Agent Response:\n{response}\n")
    
    # Test 3: Create file
    print("\n3. Testing create_file through agent...")
    request = AgentRequest(user_prompt="Create a new file called output/agent_test.txt with the content 'This file was created by the agent using file tools!'")
    response = await route_request(request, tools=file_tools)
    print(f"Agent Response:\n{response}\n")
    
    # Test 4: Edit file
    print("\n4. Testing edit_file through agent...")
    request = AgentRequest(user_prompt="Add a timestamp to the end of output/agent_test.txt")
    response = await route_request(request, tools=file_tools)
    print(f"Agent Response:\n{response}\n")
